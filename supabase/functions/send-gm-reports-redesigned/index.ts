...
import { generateWeeklyPulseHTML, generateCriticalAlertHTML, generateAllClearHTML } from './html-templates.ts'
...
serve(async (req) => {
  ...
  try {
    ...
    if (report_type === 'morning_briefing') {
      const result = await generateMorningBriefing(supabase, tenant_id, tenant.name)
      emailContent = result.content
      subject = result.subject
    } else if (report_type === 'weekly_pulse') {
      const result = await generateWeeklyPulse(supabase, tenant_id, tenant.name)
      emailContent = result.content
      subject = result.subject
    } else if (report_type === 'critical_alert') {
      const result = await generateCriticalAlert(supabase, tenant_id, tenant.name)
      emailContent = result.content
      subject = result.subject
    } else {
...
function getTrafficLight(score: number, thresholds: {good: number, warning: number}): string {
...
async function generateMorningBriefing(supabase: any, tenantId: string, tenantName: string) {
  ...
  // Get overnight data
  const overnightData = await getOvernightSnapshot(supabase, tenantId, yesterday)
  ...
  const subject = `🌅 ${tenantName} - Morning Briefing • ${today.toLocaleDateString('en-GB', {
    day: '2-digit',
    month: 'short',
    year: 'numeric'
  })}`
...
async function getOvernightSnapshot(supabase: any, tenantId: string, yesterday: Date) {
  ...
  const moderateAttentionIssues = newFeedback?.filter(f =>
    f.rating === 4 && f.comment && f.comment.length > 20
  ).map(f => ({
    guest: f.guest_name || 'Anonymous Guest',
    category: f.category,
    rating: f.rating,
    comment: f.comment.length > 80 ? f.comment.substring(0, 80) + '...' : f.comment
  })) || []
...
async function getPriorityActions(supabase: any, tenantId: string) {
  ...
  urgentFeedback?.forEach(feedback => {
    actions.push({
      type: 'guest_issue',
      priority: 'high',
      description: `${feedback.rating}⭐ ${feedback.category} issue - ${feedback.guest_name}`,
...
async function getRatingProgress(supabase: any, tenantId: string) {
  ...
  const breakdown = breakdownData?.[0]?.rating_breakdown || {
    excellent: 59, good: 43, average: 21, poor: 5, terrible: 11
  }
...
async function getTodaysFocus(supabase: any, tenantId: string) {
  ...
  const topIssue = Object.entries(categoryIssues).sort(([,a], [,b]) => b - a)[0]

  if (topIssue) {
    focus.push(`Monitor ${topIssue[0]} department - ${topIssue[1]} recent issues`)
  }
    ...
    <body>
      <div class="container">
        <div class="header">
          ...
          <p>${tenantName} • ${new Date().toLocaleDateString('en-GB', {
            weekday: 'long',
            day: '2-digit',
            month: 'long',
            year: 'numeric'
        ...
        <div class="section">
          ...
          <div class="metric-grid">
            ...
            <div class="metric">
              <div class="metric-value ${data.overnight.urgentIssues > 0 ? 'text-red-600' : ''}">${data.overnight.urgentIssues}</div>
        ...
        <div class="section">
          ...
          <div class="progress-container">
            ...
            <div style="margin-top: 12px; font-size: 14px; color: #64748b;">
              <strong>Strategic Target:</strong> Convert ${(data.rating.conversionStrategy?.convert_4_to_5 || 0) + (data.rating.conversionStrategy?.convert_3_to_5 || 0) + (data.rating.conversionStrategy?.convert_2_to_5 || 0) + (data.rating.conversionStrategy?.convert_1_to_5 || 0)} existing ratings + ${data.rating.reviewsNeeded} new 5⭐ reviews
            </div>
          </div>
        </div>
        ...
        ${data.overnight.moderateAttentionIssues && data.overnight.moderateAttentionIssues.length > 0 ? `
        <div class="section">
          <h2 class="section-title">
            <span class="emoji">🟡</span>
          ...
          ${data.overnight.moderateAttentionIssues.map((issue, index) => `
            <div class="focus-item" style="background: #fef3c7; border-left: 3px solid #f59e0b; padding: 12px; margin: 8px 0;">
              <strong>${index + 1}. ${issue.guest} - ${issue.category}</strong> (${issue.rating}⭐)<br>
...
async function generateWeeklyPulse(supabase: any, tenantId: string, tenantName: string) {
  const today = new Date()
  const lastWeek = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
  ...
  const departmentScores = await getDepartmentScorecard(supabase, tenantId, lastWeek)

  // Get strategic attention items
  const attentionItems = await getStrategicAttentionItems(supabase, tenantId, lastWeek)

  // Get wins to celebrate
  const wins = await getWinsToCelebrate(supabase, tenantId, lastWeek)

  const subject = `📈 ${tenantName} - Weekly Performance Pulse • Week ${getWeekNumber(today)}`
...
async function generateCriticalAlert(supabase: any, tenantId: string, tenantName: string) {
  console.log('🚨 Checking for critical issues requiring immediate GM attention...')
  ...
  if (criticalIssues.length === 0) {
    ...
    const subject = `✅ ${tenantName} - All Clear • ${new Date().toLocaleDateString('en-GB')}`
...
// Get department scorecard
async function getDepartmentScorecard(supabase: any, tenantId: string, lastWeek: Date) {
  ...
  const departmentScores = departments.map(dept => {
    ...
    const categoryMappings = {
      'Housekeeping': ['Cleanliness', 'Room Condition', 'Housekeeping', 'Room Quality'],
      'Front Desk': ['Staff Service', 'Check-in', 'Reception', 'Front Desk', 'Check-out'],
      'Food & Beverage': ['Dining', 'Restaurant', 'Food', 'Breakfast', 'Food & Beverage', 'Bar'],
      'Maintenance': ['Facilities', 'Maintenance', 'Room Maintenance', 'Equipment', 'Repairs'],
      'Guest Relations': ['Guest Service', 'Overall Experience', 'Guest Relations', 'Customer Service']
    }
    ...
    console.log(`📊 ${dept.name}: Found ${categoryFeedback.length} relevant feedback items`)
    ...
    if (hasData) {
      avgRating = categoryFeedback.reduce((sum, f) => sum + f.rating, 0) / categoryFeedback.length
    }

    const trend = hasData ? getTrendArrow(avgRating, 4.0) : '➡️'
    const status = hasData ? getTrafficLight(avgRating, { good: 4.0, warning: 3.5 }) : '⚪'
...
// Get strategic attention items
async function getStrategicAttentionItems(supabase: any, tenantId: string, lastWeek: Date) {
  const items = []
  ...
  Object.entries(categoryIssues)
    ...
    .forEach(([category, count]) => {
      items.push({
        ...
        description: `${count} guest complaints this week - pattern analysis needed`,
...
// Get wins to celebrate
async function getWinsToCelebrate(supabase: any, tenantId: string, lastWeek: Date) {
  const wins = []
  ...
  fiveStarFeedback?.forEach(feedback => {
    wins.push({
      ...
      description: `"${feedback.comment?.substring(0, 100)}..." - ${feedback.guest_name}`,
...
async function getCriticalIssues(supabase: any, tenantId: string) {
  ...
  const { data: criticalFeedback } = await supabase
    .from('feedback')
    .select('id, guest_name, guest_email, rating, category, comment, created_at')
    .eq('tenant_id', tenantId)
    .lte('rating', 3)
    .gte('created_at', last24Hours.toISOString())
    .order('created_at', { ascending: false })

  console.log(`Found ${criticalFeedback?.length || 0} critical feedback entries`)
  ...
  if (recentRatings && recentRatings.length >= 1) {
    ...
    const hoursOld = (now.getTime() - latestScrape.getTime()) / (1000 * 60 * 60)

    // Flag if data is older than 8 hours (beyond normal 6-hour cycle)
    if (hoursOld > 8) {
      dataFreshnessIssue = true
      console.log(`⚠️ TripAdvisor data freshness issue: ${hoursOld.toFixed(1)} hours old`)
    }
    ...
    if (recentRatings.length >= 2) {
      ...
      console.log(`Rating trend: ${previous}⭐ → ${current}⭐ (Drop: ${ratingDropDetected})`)
    }
  }
  ...
  criticalFeedback?.forEach(feedback => {
    issues.push({
      ...
      title: `${feedback.rating}⭐ Guest ${feedback.rating <= 2 ? 'Complaint' : 'Concern'} - ${feedback.category}`,
      description: `Guest ${feedback.guest_name || 'Anonymous'} submitted a ${feedback.rating}-star review about ${feedback.category}. ${feedback.rating <= 2 ? 'High risk of negative public review.' : 'Service improvement opportunity identified.'}`,
      guestImpact: feedback.rating <= 2 ? 'High - Potential TripAdvisor review risk' : 'Medium - Service quality concern',
      actions: [
      ...
      contacts: [
        { name: 'Front Desk Manager - David Mensah', phone: '+233 24 456 7890' },
  ...
  if (ratingDropDetected) {
    issues.push({
      ...
      description: `Rating decreased from ${recentRatings[1].rating}⭐ to ${recentRatings[0].rating}⭐`,
  ...
  if (dataFreshnessIssue) {
    issues.push({
      ...
      description: `Rating data is ${Math.round((now.getTime() - new Date(recentRatings[0].scraped_at).getTime()) / (1000 * 60 * 60))} hours old (beyond 6-hour update cycle)`,
