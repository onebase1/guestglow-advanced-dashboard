...
serve(async (req) => {
  ...
  try {
    ...
    if (report_type === "daily") {
      ...
      const result = await generateUrgentAlert(
        supabase,
        tenant_id,
        tenant.name,
      );
    ...
    try {
      ...
      console.warn("GM report logging failed (non-blocking):", e);
    }

    // Also log to communication_logs per recipient for dashboard metrics
    try {
      const logs = (recipient_emails || []).map((to) => ({
        tenant_id,
        email_type: `${report_type}_report`,
        recipient_email: to,
      ...
      if (logs.length > 0) {
        const { error: commErr } = await supabase.from("communication_logs")
          .insert(logs);
        if (commErr) {
          console.warn(
            "GM report comm_logs insert failed (non-blocking):",
            commErr,
          );
        }
      }
    } catch (e) {
      console.warn("GM report comm_logs logging error (non-blocking):", e);
    }
...
async function generateWeeklyReport(
  ...
  const converted =
    conversionData?.filter((item: any) =>
      item.conversion_status === "converted"
    ).length || 0;

  const subject = `🔍 ${tenantName} - Weekly Issues Analysis • ${
    new Date().toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "short",
      year: "numeric",
...
<body>
    <div class="email-container">
        <div class="header">
            ...
            <div style="color: #6b7280; font-size: 16px;">Week Ending: ${
    new Date().toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "long",
      year: "numeric",
    })
  } • Comprehensive Pattern Report</div>
        </div>
    ...
    Object.keys(issuesByCategory).length === 0
      ? "<p>✅ No significant recurring issues detected this week!</p>"
      : ""
  }
        </div>
        ...
        <div class="positive-section">
            ${
    Object.entries(positiveByCategory).slice(0, 3).map((
      [category, count]: [string, any],
        ...
        <div class="conversion-section">
            ...
            <p><strong>External Conversion Rate:</strong> ${
    Math.round((converted / totalTracked) * 100)
  }% (${converted} converted)</p>
            <p><strong>Potential Gain:</strong> +${
    totalTracked - converted
  } additional five-star reviews</p>
        </div>
        ...
        <p style="margin-top: 30px;"><strong>Next Report: Monday, ${
    new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString("en-GB", {
      ...
      year: "numeric",
        ...
        <div class="footer">
            ...
            <p>This automated report was generated at ${
...
async function generateUrgentAlert(
  supabase: any,
  tenantId: string,
  tenantName: string,
) {
  ...
  const alert = {
    ...
    manager_summary:
      `TripAdvisor rating dropped from ${dropDetails.from}⭐ to ${dropDetails.to}⭐ (${dropDetails.drop} point drop)`,
  ...
  const subject =
    `🚨 URGENT: ${tenantName} Rating Drop Detected - Immediate Action Required`;
...
<body>
    <div class="email-container">
        ...
        <div class="recovery-section">
            ...
            <p><strong>Reviews needed for recovery:</strong> ${
    Math.ceil(alert.dropDetails.drop * 20)
  } five-star reviews</p>
            <p><strong>Current daily average:</strong> 1.9 five-star reviews</p>
            <p><strong>Recovery timeline:</strong> ${
    Math.ceil((alert.dropDetails.drop * 20) / 1.9)
  } days at current pace</p>
