...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Scheduled check at ${now.toISOString()}: Hour=${hour}, Day=${dayOfWeek}, Minute=${minute}`,
    );
...
async function sendGMReport(reportType: "daily" | "weekly" | "urgent") {
  try {
    const response = await fetch(
      `${Deno.env.get("SUPABASE_URL")}/functions/v1/send-gm-reports`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to send ${reportType} report: ${response.status}`,
      );
    }
...
async function checkUrgentAlerts(supabase: any) {
  try {
    ...
    if (progress && progress.rating_change < -0.1) {
      console.log(
        `Significant rating drop detected: ${progress.rating_change}`,
      );
      ...
      if (!existingAlert) {
        await supabase
          ...
          .insert({
            ...
            manager_summary:
              `Rating drop detected: ${progress.rating_change} stars`,
