...
serve(async (req) => {
  ...
  try {
    ...
    const { feedback_text, rating, response_text, tenant_id, feedback_id }: RiskAssessmentRequest = await req.json()

    // Perform risk assessment
    const assessment = await assessResponseRisk(feedback_text, rating, response_text)
...
async function assessResponseRisk(
  ...
  if (hasHealthSafetyCritical(feedback)) {
    ...
    explanation += "Serious health or safety issue requiring medical attention. "
  }
  ...
  if (hasSecurityIncident(feedback)) {
    ...
    explanation += "Security incident involving theft, assault, or unauthorized access. "
  }
  ...
  const severity_level: 'HIGH' | 'MEDIUM' | 'LOW' = 
  ...
  const ai_confidence_score = Math.max(0.7, Math.min(1.0, (riskScore / 100) + 0.3))

  if (!explanation) {
    explanation = "Routine service complaint - low risk for automated response."
  }
...
function hasLegalThreat(text: string): boolean {
  const legalKeywords = [
    'lawsuit', 'sue', 'suing', 'lawyer', 'attorney', 'legal action',
    'discrimination', 'harassment', 'civil rights', 'ada violation',
    'health department', 'regulatory', 'compliance violation'
  ]
...
function hasHealthSafetyCritical(text: string): boolean {
  const healthSafetyKeywords = [
    'food poisoning', 'hospital', 'emergency room', 'medical treatment',
    'ambulance', 'injury', 'hurt', 'fire hazard', 'gas leak',
    'electrical', 'structural damage', 'ceiling fell', 'balcony collapse'
  ]
...
function hasSeriousStaffMisconduct(text: string): boolean {
  const misconductKeywords = [
    'theft', 'stealing', 'stole', 'drunk', 'intoxicated', 'drugs',
    'fight', 'assault', 'hit me', 'pushed me', 'threatened',
    'shared my information', 'privacy breach', 'bribery', 'corruption'
  ]
...
function hasMediaThreat(text: string): boolean {
  const mediaKeywords = [
    'viral', 'social media', 'facebook', 'twitter', 'instagram', 'tiktok',
    'news', 'reporter', 'journalist', 'boycott', 'influencer',
    'followers', 'expose', 'blast', 'shame'
  ]
...
function hasBypassAttempt(text: string): boolean {
  const bypassKeywords = [
    'ignore previous', 'system prompt', 'admin', 'root', 'sudo',
    'execute', 'command', 'script', 'function', 'override',
    'bypass', 'hack', 'inject'
  ]
...
function hasSecurityIncident(text: string): boolean {
  const securityKeywords = [
    'assault', 'attacked', 'robbed', 'stolen', 'theft',
    'unauthorized access', 'broke into', 'violence', 'weapon',
    'gun', 'knife', 'threatened', 'stalked'
  ]
