...
serve(async (req) => {
  ...
  try {
    ...
    const subject = `Thank you for your feedback regarding your recent stay in Room ${request.room_number || 'N/A'} at ${hotelName}`
...
function generateStructuredEmailContent(params: {
  ...
  if (!structuredResponse || structuredResponse.length > 300) {
    if (rating <= 3) {
      structuredResponse = `We sincerely apologize that your experience did not meet your expectations. Your feedback is invaluable to us, and we take every comment seriously.
