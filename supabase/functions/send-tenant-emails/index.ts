...
serve(async (req) => {
  ...
  try {
    ...
    // TEMP SAFETY: Block sending to hotel domains until loop is verified
    const BLOCKED_DOMAINS = ["eusbetthotel.com", "eusbetthotels.com"];
    const isBlocked = (addr?: string) =>
      !!addr &&
      BLOCKED_DOMAINS.some((d) => addr.toLowerCase().endsWith("@" + d));
    const anyBlocked = isBlocked(emailRequest.recipient_email) ||
      (emailRequest.cc_emails || []).some(isBlocked) ||
      (emailRequest.bcc_emails || []).some(isBlocked);
    if (anyBlocked) {
      console.warn(
        "🛑 Blocked outbound email to restricted domain during testing:",
        {
          recipient: emailRequest.recipient_email,
      ...
      return new Response(
        JSON.stringify({
          ...
          message: "Sending to this domain is disabled during testing",
        }),
        {
    ...
    const hotelName = emailRequest.tenant_slug
      ? emailRequest.tenant_slug.charAt(0).toUpperCase() +
        emailRequest.tenant_slug.slice(1) + " Hotel"
      : "Hotel Team";
    ...
    const emailPayload = {
      ...
      reply_to: emailRequest.email_type === "detailed_thankyou"
        ? "guestrelations@guest-glow.com"
        : undefined,
    };
    ...
    if (!resendResponse.ok) {
      ...
      throw new Error(
        `Resend API error: ${resendResponse.status} - ${errorText}`,
      );
    }
...
async function logEmailCommunication(
  ...
  try {
    const logEntry = {
      ...
      email_html: emailRequest.html_content,
      message_type: "email",
      direction: "outbound",
      status: "sent",
