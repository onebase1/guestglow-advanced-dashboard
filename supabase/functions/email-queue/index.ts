...
async function processEmailQueue(supabase: any, request: QueueRequest) {
  ...
  for (const item of queueItems || []) {
    // Human-in-loop gating: if this email relates to a feedback requiring approval and not approved, skip
    const humanInLoopEnabled =
      (Deno.env.get("HUMAN_IN_LOOP_ENABLED") ?? "false").toLowerCase() ===
        "true";
    if (humanInLoopEnabled && item.metadata?.feedback_id) {
      const { data: approvalCheck } = await supabase
        .from("response_approvals")
        .select("status, requires_approval")
        .eq("feedback_id", item.metadata.feedback_id)
        .eq("tenant_id", item.tenant_id)
        .single();
      if (
        approvalCheck?.requires_approval && approvalCheck.status !== "approved"
      ) {
        console.log(
          `⏸️ Email ${item.id} gated by human approval (status: ${approvalCheck.status})`,
        );
        await supabase
          .from("email_queue")
          .update({
            ...
            error_message:
              `Waiting for human approval (status: ${approvalCheck.status})`,
          })
          .eq("id", item.id);
        results.push({
          queue_id: item.id,
    ...
    try {
      ...
      const nextRetryTime = shouldRetry
        ? new Date(Date.now() + Math.pow(2, item.attempts) * 60000)
          .toISOString() // 2^attempts minutes
        : null;
...
async function getQueueStatus(supabase: any, request: QueueRequest) {
  ...
  for (const row of data || []) {
    ...
    priorityCounts[row.priority] = (priorityCounts[row.priority] || 0) +
      row.count;
  }
  ...
  return {
    ...
    total_queued: Object.values(statusCounts).reduce(
